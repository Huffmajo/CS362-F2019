...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case minion:
		return minionEffect(state, choice1, handPos, currentPlayer);
    ...
    case tribute:
		return tributeEffect(state, handPos, currentPlayer);
...
* choice1: decision to discard an estate for +4 coins, or gain an estate
*			1: choose to discard an estate and gain 4 coins
*			0: choose not to discard an estate
...
*			1: +2 coins
*			0: discard hand, +4 cards. All opponents with
*			   >=5 cards also do this
...
int ambassadorEffect(struct gameState *state, int choice1, int choice2, int handPos, int currentPlayer)
{
    ...
    for (int j = 0; j < choice2; j++)
    {
        for (int i = 0; i < state->handCount[currentPlayer]; i++)
        {
            if (state->hand[currentPlayer][j] == state->hand[currentPlayer][choice1])
            {
...
* Returns 0 on success, -1 on failure
************************************************************/
int tributeEffect(struct gameState *state, int handPos, int currentPlayer)
{
	// setup some necessary local variables
    ...
    for (int i = 0; i <= 2; i ++) {
		if (cardType(tributeRevealedCards[i]) == 0) //treasure card
		{
            state->coins += 2;
        }

		else if (cardType(tributeRevealedCards[i]) == 2) //victory card
		{
