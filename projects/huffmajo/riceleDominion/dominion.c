...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
    mineEffect(handPos, currentPlayer, state, choice1, choice2);
    ...
    case minion:
        minionEffect(handPos, currentPlayer, state, choice1, choice2);
    ...
    case ambassador:
        ambassadorEffect(handPos, currentPlayer, state, choice1, choice2);
...
/* 
Baron card text:  +1 Buy.  You may discard an estate for +4.  If you don't, gain an Estate.
*/
int baronEffect(int currentPlayer, struct gameState* state, int choice1) 
{
	state->numBuys++;//Increase buys by 1

	if (choice1 > 0) { //Boolean true indicates player would like to discard an estate
		...
		while (card_not_discarded) {
			...
			if (state->hand[currentPlayer][p] == estate) { //Found an estate card!
				...
				if (supplyCount(estate, state) > 0) {
                    /*
                    * Intentional bug introduced here.
                    * Changed gainCard(estate, state, 0, currentPlayer); to 
                    * gainCard(estate, state, 1, currentPlayer);
                    */
					gainCard(estate, state, 1, currentPlayer);
                    state->supplyCount[estate]--;//Decrement Estates
                    checkEstateSupply(estate, state);
				}
				card_not_discarded = 0;//Exit the loop
			}
        ...
        /*
        * Intentional bug introduced here.
        * Changed if (supplyCount(estate, state) > 0) { to
        * if (supplyCount(estate, state) >= 0) {
        */
		if (supplyCount(estate, state) >= 0) {
...
* draws 4 cards 
*/
int minionEffect(int handPos, int currentPlayer, struct gameState* state, int choice1, int choice2)
{
    int i = 0;
    ...
    * Added choice2 = 1;
    * 
    * Also changed else if (choice2) to 
    * if (choice2)
    */
    choice2 = 1;
    if (choice2)   //discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
    {
        ...
        /*
        * Intentional bug introduced here.
        * Changed for (i = 0; i < state->numPlayers; i++) to 
...
*/
int ambassadorEffect(int handPos, int currentPlayer, struct gameState* state, int choice1, int choice2){
        int i = 0;
        int availableToDiscard = 0;		//used to check if player has enough cards to discard

        /*
        * Intentional bug introduced here.
        * Changed if (choice1 == handPos || choice2 > 2 || choice2 < 0)
        * to if (choice2 > 2 || choice2 < 0)
        */
        ...
        for (i = 0; i < state->handCount[currentPlayer]; i++)
        {
            ...
            * to if (i == state->hand[currentPlayer][choice1] && i != choice1)
            */
        ...
        // Ensure enough cards are available in players hand to discard
        if (availableToDiscard < choice2)
        {
            return -1;
        }
        ...
        /*
        * Intentional bug introduced here.
        * Removed for if (i != currentPlayer){
        * and closing bracket
        */
        for (i = 0; i < state->numPlayers; i++)
        {
...
/* 
* Tribute card text: The player to your left reveals then discards the top
...
int tributeEffect(int currentPlayer, int nextPlayer, struct gameState* state){
    ...
    int tributeRevealedCards[2] = {-1, -1}; // Holds cards revealed by the next player
    ...
    if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1) {
        ...
        // Set revealed cards
        for(i = 0; i < 2; i ++){
        tributeRevealedCards[i] = state->deck[nextPlayer][state->deckCount[nextPlayer]-1]; //Set 1st revealed card
        state->deck[nextPlayer][state->deckCount[nextPlayer]--] = -1; //Set top of nextPlayer's deck to invalid card
        state->deckCount[nextPlayer]--; //Decrease the amount of cards nextPlayer has in their deck by one
        }
    }

        /*
        * Intentional bug introduced here.
        * Changed tributeRevealedCards[1] = -1;
        * to tributeRevealedCards[1] = 0;
        */
    if (tributeRevealedCards[0] == tributeRevealedCards[1]) { //If we have a duplicate card, just drop one
        state->playedCards[state->playedCardCount] = tributeRevealedCards[1];
        state->playedCardCount++;
        tributeRevealedCards[1] = 0;
    }

    /*
    * Fixed array overflow bug by changing
    * for (i = 0; i <= 2; i ++) { to
    * for (i = 0; i < 2; i ++) {
    */
    for (i = 0; i < 2; i ++) {
        ...
        tributeRevealedCards[i] == great_hall) { //Victory Card Found
            /*
            * Intentional bug introduced here.
            * Added discardCard(i, currentPlayer, state, 0);
            * and drawCards(currentPlayer, 2, state);
            */
            drawCards(currentPlayer, 2, state);
            discardCard(i, currentPlayer, state, 0);
            drawCards(currentPlayer, 2, state);
        }
...
* than it.
*/
int mineEffect(int handPos, int currentPlayer, struct gameState* state, int choice1, int choice2){
    int cardToTrash = state->hand[currentPlayer][choice1];
    int i = 0;

    /*
    * Intentional bug introduced here.
    * Changed (state->hand[currentPlayer][choice1] < copper || state->hand[currentPlayer][choice1] > gold)
    * to (state->hand[currentPlayer][choice1] < copper || state->hand[currentPlayer][choice1] >= gold)
    */
    ...
    // Changed if (choice2 > treasure_map || choice2 < curse)
    // to if (choice2 > copper || choice2 < gold)
    // Based on bug find post/fix by Eric Morgan via piazza
    if (choice2 > copper || choice2 < gold)
    {
        return -1;
    }

    // Changed greater than to less than sign.
    // Makes purchase cost comparison check correct.
    // Based on bug find post/fix by Robert Saraceno via piazza
    ...
    /*
    * Intentional bug introduced here.
    * Changed discardCard(handPos, currentPlayer, state, 0);
    * to discardCard(handPos, currentPlayer, state, 1);
    */
    //discard mine card from hand
    discardCard(handPos, currentPlayer, state, 1);

    //Trash selected treasure card
    for (i = 0; i < state->handCount[currentPlayer]; i++)
    {
        if (state->hand[currentPlayer][i] == cardToTrash)
        {
            // Changed 4th parameter in discardCard from 0 to 1.
            // This fixed a bug where card was being placed in discard and not trash pile.
            // Based on bug find post/fix by T Grasse via piazza
            discardCard(i, currentPlayer, state, 1);
            break;
        }
    }
...
/* Draws a specified number of cards and places them in the player's hand */
int drawCards(int player, int cardsToDraw, struct gameState* state){
    int j = 0;
    for (j = 0; j < cardsToDraw; j++)
    {
...
* returns 0. USED FOR RANDOM TESTING
************************************************************/
int testPass(int expected, int actual)
{
	int result = 0;
